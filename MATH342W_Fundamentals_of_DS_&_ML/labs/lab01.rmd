...
```{r}
#TO-DO
x_int = as.integer(x_factor)
class(x_int)
```

* Use `x` to create a new vector `y2` containing only the real numbers in x.
...
?split
class(copy_of_R)
# split(<data_frame>, <how are we going to split it>)
cols = split(copy_of_R, col(copy_of_R))
cols
```
...
result_list = lapply(copy_of_R, function(col) {
  list(
    min = min(col, na.rm = TRUE),
    max = max(col, na.rm = TRUE),
    pct_missing = mean(is.na(col)),
    first_NA = which.max(is.na(col))
  )
})
...
quantile_5 = quantile(v, probs=.05)
qnorm_5th = qnorm(p=0.05, mean=-10, sd=sqrt(100))

cat("Quantile: ", quantile_5, "\n", "Qnorm: ", qnorm_5th)
...
#TO-DO
my_ecdf = ecdf(v)(0)
theoretical = pnorm(0, mean = -10, sd=sqrt(100))

cat("Empirical: ", my_ecdf, "\n", "Theoretical: ", theoretical, "\n")
```
...
```{r}
#TO-DO
# I USED A TEST VEC TO SEE ALL THE RESULTS
test = c(1:10)
cat("Before my_rev()", test)
my_reverse = function(x){
  return(length(x):1)
  # starts counting down fron length of x all the way to 1,
  # reversing the list
}
cat("Before my_rev()", test,"\n", "After my_rev()", my_reverse(test))
```
...
```{r}
#TO-DO
flip_matrix = function(input_matrix, dim_to_vec=NULL){
   if (is.null(dim_to_vec)) {
    dim_to_rev <- ifelse(nrow(input_matrix) >= ncol(input_matrix), 1, 2)
  }
  if (dim_to_vec == 1) {
    return(input_matrix[nrow(input_matrix):1, , drop = FALSE])
  } 
  else if (dim_to_vec == 2) {
    return(input_matrix[, ncol(input_matrix):1, drop = FALSE])
  }
}

test_data = c(rep(1:25))
test = matrix(test_data, nrow = 5, ncol=5)
...
#TO-DO
entries = (1:8)
my_list = lapply(entries, function(i) array(1:(i^i), dim = rep(i, i)))
names(my_list) = LETTERS[1:8]
my_list
...
# Split the 'lorem' string into sentences 
sentences = unlist(strsplit(lorem, "\\."))

# Remove whitespace
sentences = trimws(sentences)

# shuffle
sentences = sample(sentences, length(sentences))
...
#TO-DO
male_boomer_names = strsplit("Theodore, Bernard, Gene, Herbert, Ray, Tom, Lee, Alfred, Leroy, Eddie", split = ", ")[[1]]
male_genx_names = strsplit("Marc, Jamie, Greg, Darryl, Tim, Dean, Jon, Chris, Troy, Jeff", split = ", ")[[1]]
male_millennial_names = strsplit("Zachary, Dylan, Christian, Wesley, Seth, Austin, Gabriel, Evan, Casey, Luis", split = ", ")[[1]]
female_boomer_names = strsplit("Gloria, Joan, Dorothy, Shirley, Betty, Dianne, Kay, Marjorie, Lorraine, Mildred", split = ", ")[[1]]
female_genx_names = strsplit("Tracy, Dawn, Tina, Tammy, Melinda, Tamara, Tracey, Colleen, Sherri, Heidi", split = ", ")[[1]]
female_millennial_names = strsplit("Samantha, Alexis, Brittany, Lauren, Taylor, Bethany, Latoya, Candice, Brittney, Cheyenne", split = ", ")[[1]]

# Create a list-within-a-list structure
names_data <- list(
  M = list(
    Boomer = male_boomer_names,
    GenX = male_genx_names,
    Millennial = male_millennial_names
  ),
  F = list(
    Boomer = female_boomer_names,
    GenX = female_genx_names,
    Millennial = female_millennial_names
  )
)
names_data
```
