...
```{r}
# pacman::p_load(Rcpp)
# evalCpp("std::cout<<'hello world;'", showOutput= TRUE)
```
...
It would make sense because the variable cut, color, and clarity are categorical. 


Would it make sense to use log transformations for the variables cut, color and clarity? Why or why not?

The same could be said. If you log(0, 1) you get infinity or 0. In short we don't transform dummies.
...
```{r}
#TO-DO
diamonds = diamonds[diamonds$carat <= 2, ] # changing the dataset to only the carats <= 2.
n = nrow(diamonds)
...
```{r}
#TO-DO
summary(lm(price ~ carat, diamonds))$coef
summary(lm(price ~ carat, diamonds))$sigma
```
...
```{r}
#TO-DO
summary(lm(price ~ clarity, diamonds))$sigma 
```

Why is the model price ~ carat substantially more accurate than price ~ clarity?

 clarity has a larger error because it is categorical and there are multiple levels
...
The data set looks a little more complex and less linear 
...
High n (rows) and low p (features) therefore there is no estimation error and the model understands the data giving a high r^2
...
Yes because it seems to be the most linear model that we can get using logs. 
...
#TO-DO
If carat size doubles, then
if carat increases by x-percent, then price increases by p-percent * 1.7 |(ln_carat = 1.696590)

Interpret b_0, the estimated intercept.

For a 1-carat diamond the price is predicted to be e^8.5
#TO-DO
...
#TO-DO : Model c performed better than model b due to the higher complexity since more features are interacted.
...
#TO-DO : D performs better than c because now ln_price is now modeled on every continuous feature unlike before it was just one feature being interacted with multiple ones. 
...
#TO-DO : E performs better since it uses more features
...
#TO-D0 : Yes it performs better in sample due to the complexity

Can you think of any other way to expand the candidate set curlyH? Discuss.

#TO-DO : There are many ways to expand curly{H} by squaring, cubing, logging, etc .. 
...
#TO-DO
#data(diamonds)
#iamonds_full = diamonds
performance = list()

# Function to calculate standard error of residuals
get_se = function(model, data) {
  residuals = resid(model)
  se = sqrt(mean(residuals^2))
  return(se)
}


for (key in c("b", "c", "d", "e", "f")){
   model_name = paste0("mod_", key)
   current_model = get(model_name)
   performance[[key]] = get_se(current_model, diamonds_full)
}
...
#TO-DO : to be better since we trained on ful ldata set. the problem is that it might not be the best and most honest since we couldve used a larger size to test.
...
#cv_result = cv.glm(diamonds, mod_f, K = k)

#cv_se = cv_result$delta

#overall_se = sqrt(mean(cv_se^2))

#s_e = sd(cv_se)

# Print the results
#print(paste("Overall s_e:", overall_se))
#print(paste("Standardized s_e:", ss_e))
...
#  // Perform Gram-Schmidt orthogonalization
 # for (int j = 1; j < p; j++) {
  #  V(_, j) = X(_, j);
   ...
   # for (int k = 0; k < j; k++) {
    #  NumericVector v_k = V(_, k);
     # NumericVector v_j = V(_, j);
      
      #// Compute the orthogonal projection of v_j onto v_k
      #NumericVector projection = (sum(v_k * v_j) / sum(pow(v_k, 2))) * v_k;
      
      #// Update v_j to be orthogonal to v_k
      #V(_, j) = v_j - projection;
    #}
  #}
  ...
  #for (int j = 0; j < p; j++) {
    #NumericVector v_j = V(_, j);
    #double norm_v_j = sqrt(sum(pow(v_j, 2)));
