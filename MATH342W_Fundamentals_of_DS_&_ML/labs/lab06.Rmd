...
```{r}
# Based off of the graphs there seems to be a continuous increase as we age, which makes sense because the older we are the more we get exposed to words which then increases vocab. There also is a threshold, at a certain age our vocabulary declines which also makes sense because the at that age our memory tends to get worse.
```
...
When all the variables are 0 the log odds are -5.504 or 0.004
...
plot(y, p_hat, xlab = "Actual Response Values", ylab = "Predicted Probabilities", main = "In-sample Probability Predictions vs. Real Response Values", col = c("blue", "green")[factor(y)])
...
#TO-DO
I'm assuming they perform about the same because their in-sample was the exact same.
